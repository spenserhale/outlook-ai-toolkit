...
const REPO = "spenserhale/outlook-ai-toolkit";
...
export function resolveAssetName(platform: string, arch: string): string | null {
  if (platform === "linux" && arch === "x64") return "outlook-linux-x64";
  if (platform === "linux" && arch === "arm64") return "outlook-linux-arm64";
  if (platform === "darwin" && arch === "x64") return "outlook-darwin-x64";
  if (platform === "darwin" && arch === "arm64") return "outlook-darwin-arm64";
  if (platform === "win32" && arch === "x64") return "outlook-windows-x64.exe";
  return null;
}
...
export function isCompiledBinary(execPath: string): boolean {
  ...
  // (basename `outlook` / `outlook.exe`). Anything else (node, bun, npx)
...
export async function runUpgrade(deps: UpgradeDeps, flags: UpgradeFlags): Promise<void> {
  if (!isCompiledBinary(deps.execPath)) {
    deps.error("`outlook upgrade` is only supported on the standalone binary install.");
    deps.error("");
    deps.error("You appear to be running via Node.js / a package manager. Update via:");
    deps.error("  npm install -g @outlook-ai-toolkit/cli@latest");
    deps.error("  bun add -g @outlook-ai-toolkit/cli@latest");
    deps.error("");
    deps.error("Or switch to the standalone binary:");
    deps.error("  curl -fsSL https://raw.githubusercontent.com/spenserhale/outlook-ai-toolkit/main/scripts/install.sh | sh");
    deps.exit(1);
  }
  ...
  if (flags.check) {
    if (cmp >= 0) {
      ...
      deps.log("Update available. Run `outlook upgrade` to install.");
    }
    return;
  }
  ...
  const tmpPath = join(binDir, `.outlook-upgrade-${process.pid}.tmp`);
