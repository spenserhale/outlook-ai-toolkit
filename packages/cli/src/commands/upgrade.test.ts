...
function makeDeps(overrides: Partial<UpgradeDeps> = {}): UpgradeDeps {
  return {
    ...
    execPath: "/usr/local/bin/outlook",
    currentVersion: "0.1.0",
    platform: "linux",
    arch: "x64",
    writeBinary: mock(async () => {}),
...
describe("resolveAssetName", () => {
  it("maps all supported platforms", () => {
    expect(resolveAssetName("linux", "x64")).toBe("outlook-linux-x64");
    expect(resolveAssetName("linux", "arm64")).toBe("outlook-linux-arm64");
    expect(resolveAssetName("darwin", "x64")).toBe("outlook-darwin-x64");
    expect(resolveAssetName("darwin", "arm64")).toBe("outlook-darwin-arm64");
    expect(resolveAssetName("win32", "x64")).toBe("outlook-windows-x64.exe");
  });
...
describe("isCompiledBinary", () => {
  it("detects compiled binaries by exec basename", () => {
    expect(isCompiledBinary("/usr/local/bin/outlook")).toBe(true);
    expect(isCompiledBinary("C:/Users/x/outlook.exe")).toBe(true);
    expect(isCompiledBinary("/home/u/.local/bin/outlook")).toBe(true);
  });
...
describe("runUpgrade", () => {
  ...
  it("reports update-available in --check mode", async () => {
    ...
    expect(deps.log).toHaveBeenCalledWith(
      "Update available. Run `outlook upgrade` to install.",
    );
    expect(deps.writeBinary).not.toHaveBeenCalled();
  });
  ...
  it("downloads, verifies, and replaces binary on real upgrade", async () => {
    ...
    const fetchMock = mock(async (input: unknown) => {
      ...
      if (url.endsWith(".sha256")) {
        return new Response(`${sha}  outlook-linux-x64\n`, { status: 200 });
      }
  ...
  it("rejects binary with mismatched sha256", async () => {
    const fetchMock = mock(async (input: unknown) => {
      ...
      if (url.endsWith(".sha256")) {
        return new Response("deadbeef  outlook-linux-x64\n", { status: 200 });
      }
